...
struct Staircase: Decodable, Hashable {
    var openings: [[Double]]
...
class API {
    static func fetch_data(onFetch: @escaping ([Int: Floor]?) -> ()) {
        guard let url = URL(string: "https://monkfish-app-4ffx2.ondigitalocean.app/lbss/map") else {
            onFetch(nil)
            return
        }
    ...
    static func find_route(start: [[Double]], end: [[Double]], elevators: Bool, onFind: @escaping (Route?) -> ()) {
        guard let url = URL(string: "https://monkfish-app-4ffx2.ondigitalocean.app/lbss/route") else {
            onFind(nil)
            return
        }
