...
struct Home: View {
    ...
    @State private var open_popup: Bool = true
    ...
    @State private var current_floor: Int = 1
        ...
        GeometryReader { screen in
            ZStack {
                ...
                if floors[current_floor] != nil {
                    ...
                    ForEach(self.floors[current_floor]!.metadata.staircases, id: \.self) { stairs in
                        Text(stairs.global_stair_index)
                            .foregroundColor(Color(red: 0.623529412, green: 0.619607843, blue: 0.611764706))
                            .minimumScaleFactor(0.1)
                            .multilineTextAlignment(.center)
                            .frame(
                                width: stairs.text_boundary[2]-stairs.text_boundary[0]-8,
                                height: stairs.text_boundary[3]-stairs.text_boundary[1]-4
                            )
                            .padding(.horizontal, 4)
                            .padding(.vertical, 2)
                            .position(
                                x: (stairs.text_boundary[0]+stairs.text_boundary[2])/2,
                                y: (stairs.text_boundary[1]+stairs.text_boundary[3])/2
                            )
                            .offset(x: translate.x, y: translate.y)
                            .scaleEffect(scale)
                    }
                    ...
                    if following_route && !route.directions.isEmpty {
                        ...
                        HStack(alignment: .top) {
                            ...
                            VStack(spacing: 0) {
                                HStack {
                                    if show_searchbar {
                                        TextField("Search LBSS", text: $search)
